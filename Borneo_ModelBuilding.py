...
file_today = 'BorneoArticles{}.csv'.format(time.strftime("%d%m%Y"))
# Read the input data
...
df.to_csv("BorneoArticles_summary{}.csv".format(time.strftime("%d%m%Y")), index=False)
...
def upload_to_s3():
    ...
    s3.Bucket('nlpnewsarticlebucket').upload_file(Filename='BorneoArticles_summary{}.csv'.format(cd),
                                                  Key='BorneoArticles_summary{}.csv'.format(cd))
